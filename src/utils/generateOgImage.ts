...
async function loadGoogleFont(font: string, text: string) {
    const cacheKey = `${font}`; // Cache by font name only to reuse across different titles
    
    if (!fontCache.has(cacheKey)) {
        const promise = (async () => {
            // Use a mirror for better accessibility in some regions
            // We use a broad character set to make the font reusable for most titles
            const charSet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.,!?-()[]{} ";
            const API = `https://google-fonts.mirrors.sjtug.sjtu.edu.cn/css2?family=${font}&text=${encodeURIComponent(charSet)}`;
            ...
            try {
                ...
                fontCache.delete(cacheKey); // Don't cache failures
                throw error;
            }
        })();
